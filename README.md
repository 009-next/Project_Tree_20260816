...
projecttree/          Project_Tree の機能モジュール
  ...
  ├─ ifcexport.py     CIM連携。台帳の記録を属性として載せたIFCを書き出す
  ...
  ├─ authguard.py     起動ごとのトークンによるAPI保護（ローカル起動時）
...
- `127.0.0.1` 固定バインド
- **起動ごとのトークンで API を保護**（`projecttree/authguard.py`）。
  アプリが開くブラウザには自動でトークンを渡すため操作は変わらないが、
  トークンを持たない要求はすべて 401 で止まる。Host も `127.0.0.1` / `localhost` に限定
  （検証などで外したい場合のみ環境変数 `PT_NO_AUTH=1`）
