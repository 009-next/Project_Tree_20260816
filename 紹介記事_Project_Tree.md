...
FastAPI + SQLite（台帳）
  ...
  ├─ modelgen.py     2D/3Dモデル生成（trimesh・LLM不使用）
...
*「基本構造の追加」「写真から作成」「図面・モデル取込」はいずれもBlender不要のローカル生成。Blender連携は下部に独立した任意ボタンとして分離してある（画像は未接続の状態）*

## 実際に動かしてみた数字

ハッカソン用のサンプル案件22件（実データを模した工事記録）で通しテストした結果です。
...
| 抽出したイベント | 431件 |
| 案件数（工事） | 40件前後（自動統合含む） |
| 生成したイメージ図 | 5枚（$0.025〜0.037/枚） |
| 推論1回あたりの実測コスト | $0.023〜0.047（Opus 5使用時） |
| 写真→3Dモデル化 | $0.0026〜0.0054/枚（Haiku 4.5使用時） |
| 累計API実測コスト | 約$2.4 |
